  ...
  return (
    <section aria-label={`${selectedOfficeName} institution register`} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-soft">
      <div className="flex flex-col justify-between gap-4 border-b border-slate-100 p-5 sm:p-6 lg:flex-row lg:items-center">
        <div className="flex items-center gap-3"><div className="rounded-xl bg-blue-100 p-3 text-blue-700"><Building2 size={23} /></div><div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-blue-700">Institution register</p><h2 className="mt-1 text-lg font-semibold tracking-tight text-slate-900">{selectedOfficeName}</h2></div></div>
        <div className="flex flex-wrap gap-2 text-xs font-medium"><span className="rounded-full bg-slate-100 px-3 py-1.5 text-slate-600">{institutions.length} records</span><span className="rounded-full bg-blue-100 px-3 py-1.5 text-blue-700">{checked} accomplished</span><span className="rounded-full bg-slate-100 px-3 py-1.5 text-slate-700">{pending} pending</span>{unknown > 0 && <span className="rounded-full bg-indigo-50 px-3 py-1.5 text-indigo-700">{unknown} unknown</span>}</div>
      </div>
      <div className="flex flex-col justify-between gap-3 border-b border-slate-200 bg-slate-50/60 px-5 py-4 sm:px-6 xl:flex-row xl:items-center">
        <div className="relative"><Search size={16} className="pointer-events-none absolute left-3 top-3 text-slate-400" /><input type="search" aria-label="Search institutions or regions" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search institutions or regions…" className="w-full rounded-lg border border-slate-200 bg-white py-2.5 pl-9 pr-3 text-sm outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-100 sm:min-w-80" /></div>
        <div role="group" aria-label="Filter by survey status" className="flex flex-wrap gap-1 rounded-lg border border-slate-200 bg-white p-1">{[["all", "All records"], ["checked", "Accomplished"], ["pending", "Pending"], ...(unknown ? [["unknown", "Unknown"]] : [])].map(([value, label]) => <button key={value} type="button" aria-pressed={status === value} onClick={() => setStatus(value)} className={`rounded-md px-3 py-2 text-xs font-medium transition focus-visible:outline-blue-600 ${status === value ? "bg-blue-700 text-white shadow-sm" : "text-slate-500 hover:bg-slate-100"}`}>{label}</button>)}</div>
      </div>
